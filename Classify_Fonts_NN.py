### CLASSIFY FONTS USING A DEEP NEURAL NETWORK : 1 Hidden layers : 67% accuracy
...
W1 = tf.Variable(tf.truncated_normal([1296, num_hidden_layers], stddev=1./math.sqrt(1296)))
b1 = tf.Variable(tf.constant(0.1, shape=[num_hidden_layers]))
hidden1_out = tf.sigmoid(tf.matmul(X, W1) + b1)

### Output layer
W2 = tf.Variable(tf.truncated_normal([num_hidden_layers, 5], stddev=1./math.sqrt(5)))
b2 = tf.Variable(tf.constant(0.1, shape=[5]))

#Initialize all
session.run(tf.initialize_all_variables())

Y = tf.nn.softmax(tf.matmul(hidden1_out, W2) + b2)
